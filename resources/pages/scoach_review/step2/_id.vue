  ...
  methods:{
    async onSubmit(){
      ...
      if(res.status == 200){
        ...
        if(this.reviewInfo.review_type == 'school') this.$router.push(`/school_coach/${this.reviewInfo.reviewFor}`)
        else if(this.reviewInfo.review_type == 'legend') this.$router.push(`/local_instructor/${this.reviewInfo.reviewFor}`)
  ...
  async asyncData({app, store,redirect, params}){
    try {
        const [res1, res2] = await Promise.all([
            app.$axios.get(`/app/attributes?atIndex=Harmful`),  
